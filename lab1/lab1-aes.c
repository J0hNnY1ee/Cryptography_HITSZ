...
int T(int num, int round)
{
    ...
    byte1 = getNumFromSBox(byte1);
    byte2 = getNumFromSBox(byte2);
    byte3 = getNumFromSBox(byte3);
    byte4 = getNumFromSBox(byte4);

    // Apply the Rcon value for the specific round
    int rcon = Rcon[round - 1]; // Adjust for 0-based index

    // Combine transformed bytes and Rcon
    int result = (byte1 & 0xff) | ((byte2 << 24) & 0xff000000) | ((byte3 << 16) & 0x00ff0000) | ((byte4 << 8) & 0x0000ff00);
    return result ^ rcon;
}
